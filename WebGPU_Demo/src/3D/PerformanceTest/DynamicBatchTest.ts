		...
		Laya.loader.load("resources/res/threeDimen/layabox.png").then((tex: Texture2D) => {
			...
			for (var i: number = 0; i < circleCount; i++) {
				...
				for (var j: number = 0; j < oneCircleCount; j++) {
					var boxSprite: Laya.Sprite3D = new Laya.Sprite3D();
					let meshfilter: Laya.MeshFilter = boxSprite.addComponent(Laya.MeshFilter);
					let meshrender: Laya.MeshRenderer = boxSprite.addComponent(Laya.MeshRenderer);
