...
export class GCA_BatchInfoManager {
    ...
    _removeList: Laya.SingletonList<batchInfoChangeInfo> = new Laya.SingletonList<batchInfoChangeInfo>();
    _changeList: Laya.SingletonList<batchInfoChangeInfo> = new Laya.SingletonList<batchInfoChangeInfo>();

    _littleHoleData: Laya.Vector2 = new Laya.Vector2(0, 0);
    _littleElementCollects: GCA_InstanceRenderElementCollect[] = [];

    _someHoleData: Laya.Vector2 = new Laya.Vector2(0, 0);
    _someElementCollects: GCA_InstanceRenderElementCollect[] = [];

    _quitHoleData: Laya.Vector2 = new Laya.Vector2(0, 0);
    _quitCountElementCollects: GCA_InstanceRenderElementCollect[] = [];

    _largeElementCollects: GCA_InstanceRenderElementCollect[] = [];

    _computeCommandBuffer: Laya.ComputeCommandBuffer;
    _cullShaderData: Laya.ShaderData;
    ...
    setCullPlanes(cullDatas: Float32Array) {
        this._cullShaderData.setBuffer(Laya.Shader3D.propertyNameToID("cullPlanes"), cullDatas);
    }

    setLodConfig(lodRangeArray: Float32Array) {
        this._cullShaderData.setBuffer(Laya.Shader3D.propertyNameToID(""), lodRangeArray);
    }
    ...
    applyChage() {
        ...
        window["openCS" as any] && this._computeCommandBuffer.executeCMDs();
    }

    appendBatch(opaque: Laya.RenderListQueue, alphaTest: Laya.RenderListQueue) {
        for (let i = 0; i < this._largeElementCollects.length; i++) {
            let collect = this._largeElementCollects[i];
