import { batchInfoChangeType } from "./GCA_InsBatchAgent";
...
export class GCA_OneBatchInfo {
    static GCA_StorageBuffer: Laya.ShaderDefine;
    static customdataStridMap: Map<string, number> = new Map();
    static customDataWholeStride: number = 0;
    static setCustomCommandmap(customAttributeData: Map<string, [number, Laya.ShaderDataType]>) {
    ...
    private _chagneIndexList: Laya.SingletonList<{ index: number, type: batchInfoChangeType }> = new Laya.SingletonList();
    ...
    renderGeometry: Laya.WebGPURenderGeometry;

    private _ibCount: number = 0;
    private _ibOffset: number = 0;

    spriteShaderData3D: Laya.WebGPUShaderData;
    ...
    private _bindOwnerRenderData() {
        ...
        this._updateIndirectDrawGeometry();
        this.spriteShaderData3D.setDeviceBuffer(Laya.Shader3D.propertyNameToID("instances"), this._owner.worldMatrixDeviceBuffer);
        this.spriteShaderData3D.setDeviceBuffer(Laya.Shader3D.propertyNameToID("customDatas"), this._owner.customDeviceBuffer);
        this.spriteShaderData3D.setDeviceBuffer(Laya.Shader3D.propertyNameToID("insIndexs"), this._owner.instanceIndexDeviceBuffer);
        Laya.Vector4.TEMP.setValue(this.maxBlockCount, this.blockIndexInOwner, 0, 0);
        this.spriteShaderData3D.setVector(Laya.Shader3D.propertyNameToID("u_cullBlockData"), Laya.Vector4.TEMP);
    }

    private _unbindOwnerRenderData() {
        //TODO  移除渲染资源绑定等
        this.spriteShaderData3D.setDeviceBuffer(Laya.Shader3D.propertyNameToID("instances"), null);
        this.spriteShaderData3D.setDeviceBuffer(Laya.Shader3D.propertyNameToID("customDatas"), null);
        this.spriteShaderData3D.setDeviceBuffer(Laya.Shader3D.propertyNameToID("insIndexs"), null);
        this.renderGeometry.clearRenderParams();
    }
    ...
    private _createRenderElement() {
        ...
        this.spriteShaderData3D = Laya.LayaGL.renderDeviceFactory.createShaderData() as Laya.WebGPUShaderData;
    ...
    protected _initRenderElement(data: batchIDInfo) {
        ...
        //mat bind
        this.renderElement.materialShaderData = material.shaderData as Laya.WebGPUShaderData;
        ...
        //geometry bind
        let geometry = this.renderGeometry = Laya.LayaGL.renderDeviceFactory.createRenderGeometryElement(Laya.MeshTopology.Triangles, Laya.DrawType.DrawElementIndirect) as Laya.WebGPURenderGeometry;
        geometry.bufferState = meshGeometry.bufferState as Laya.WebGPUBufferState;
        ...
        this.renderElement.geometry = geometry as Laya.WebGPURenderGeometry;
    }
    ...
    protected _initShaderDataValue(data: batchIDInfo) {
        if (data.isReceiveShadow) {
            this.spriteShaderData3D.addDefine(Laya.RenderableSprite3D.SHADERDEFINE_RECEIVE_SHADOW);
        } else {
            this.spriteShaderData3D.removeDefine(Laya.RenderableSprite3D.SHADERDEFINE_RECEIVE_SHADOW);
        }
