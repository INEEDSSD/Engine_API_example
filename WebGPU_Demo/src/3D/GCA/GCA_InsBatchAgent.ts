...
export class GCA_InsBatchAgent {
    ...
    private _cameraCullSingleList: Laya.SingletonList<IGCABVHCell> = new Laya.SingletonList<IGCABVHCell>();

    private _dirShadowCullSingleList: Laya.SingletonList<IGCABVHCell> = new Laya.SingletonList<IGCABVHCell>();
    ...
    constructor(hasDirShadow: boolean = true) {
        ...
        if (hasDirShadow) {
            this._dirShadowCullSingleList = new Laya.SingletonList<IGCABVHCell>();
        }
    }
     ...
     * @returns 
     */
    setCullingCamera(camera: Laya.Camera): Laya.SingletonList<IGCABVHCell> {
        //准备所有的渲染节点
     ...
     * @returns 
     */
    setCullingDir(cullInfo: Laya.ShadowCullInfo): Laya.SingletonList<IGCABVHCell> {
        this._dirShadowCullSingleList.length = 0;
        for (let i = 0; i < this._unLoadInsArray.length; i++) {
            let ins = this._unLoadInsArray[i];
            if ((ins.renderMask & GCARenderMask.CastShadow) && Laya.FrustumCulling.cullingRenderBounds(ins.bounds, cullInfo)) {
    ...
    getRenderList(manager: GCA_BatchInfoManager, opaque: Laya.RenderListQueue, alphaTest: Laya.RenderListQueue) {
     ...
     */
    appendBatch(cullMode: CullingMode, opaque: Laya.RenderListQueue, alphaTest: Laya.RenderListQueue, transparent?: Laya.RenderListQueue, transparentAfter?: Laya.RenderListQueue) {
        //进行Gpu Cull
        //数据更新
        //
    }
