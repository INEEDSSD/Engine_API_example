export class GCA_BatchRenderElement extends (Laya.WebGPURenderElement3D ? Laya.WebGPURenderElement3D : Laya.WebGLRenderElement3D) {
   ...
   * @param context 
   */
    protected _compileShader(context: Laya.WebGPURenderContext3D) {
        this._shaderInstances.clear();
        let comDef = this._getShaderInstanceDefines(context);

        //查找着色器对象缓存
        var passes: Laya.ShaderPass[] = this.subShader._passes;
        for (var j: number = 0, m: number = passes.length; j < m; j++) {
            let pass = passes[j];
            let passdata = <Laya.WebShaderPass>pass.moduleData;
    ...
    _render(context: Laya.WebGPURenderContext3D, command: Laya.WebGPURenderCommandEncoder | Laya.WebGPURenderBundle) {
        ...
        for (let j: number = 0, m: number = (this as any)._shaderInstances.length; j < m; j++) {
            ...
            if (!command.isBundle && this.depthStencilParam.stencilEnable) {
                (command as Laya.WebGPURenderCommandEncoder).setStencilReference(this.depthStencilParam.stencilRef);
            }
    ...
    protected _bindGroup(context: Laya.WebGPURenderContext3D, shaderInstance: Laya.WebGPUShaderInstance, command: Laya.WebGPURenderCommandEncoder | Laya.WebGPURenderBundle) {
        ...
        }
        {
            ...
            let bindgroup = Laya.WebGPURenderEngine._instance.bindGroupCache.getBindGroup([this.subShader._owner.name], this.materialShaderData, null, shaderResource, textureExitsMask);
