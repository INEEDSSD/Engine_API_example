...
export class Shader_GlowingEdge extends BaseScript {
	...
	onComplete(res: any[]): void {
		...
		//创建平行光
		var directionLight: Laya.Sprite3D = new Laya.Sprite3D();
		var directionLightCom: Laya.DirectionLightCom = directionLight.addComponent(Laya.DirectionLightCom);
		...
		(<Laya.SkinnedMeshSprite3D>dude.getChildAt(0).getChildAt(0)).getComponent(Laya.SkinnedMeshRenderer).sharedMaterials = baseMaterials;
		...
		//加载地球精灵
		var earth: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createSphere(0.5, 128, 128), null);
