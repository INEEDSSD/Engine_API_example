...
export class MouseInteraction extends BaseScript {
	...
	private onComplete(): void {
		...
		grid.layer = 10;
		//加载静态小猴子
		var staticLayaMonkey: Laya.Sprite3D = new Laya.Sprite3D();
		let mesh: Laya.Mesh = Loader.getRes("resources/res/threeDimen/skinModel/LayaMonkey/Assets/LayaMonkey/LayaMonkey-LayaMonkey.lm") as Laya.Mesh;
		...
		let mat = Loader.getRes("resources/res/threeDimen/skinModel/LayaMonkey/Assets/LayaMonkey/Materials/T_Diffuse.lmat") as Laya.Material;
		//设置材质
		...
		var layaMonkey_clone1: Laya.Sprite3D = (<Laya.Sprite3D>Sprite3D.instantiate(staticLayaMonkey, this.scene, false, this.tmpVector));
		var layaMonkey_clone2: Laya.Sprite3D = (<Laya.Sprite3D>Sprite3D.instantiate(staticLayaMonkey, this.scene, false, this.tmpVector));
		var layaMonkey_clone3: Laya.Sprite3D = (<Laya.Sprite3D>Sprite3D.instantiate(staticLayaMonkey, this.scene, false, this.tmpVector));
		//设置精灵名字
		...
		// layaMonkey_clone3.transform.setWorldLossyScale(scale);
		...
		//获取模型的mesh
		meshShape.mesh = (<Mesh>staticLayaMonkey.getComponent(Laya.MeshFilter).sharedMesh);
		//设置模型的碰撞形状
...
class SceneScript extends Laya.Script {
	private meshSprite: Laya.Sprite3D;
	...
	onAwake(): void {
		this.meshSprite = (<Laya.Sprite3D>this.owner);
	...
	onCollisionEnter(collision: Laya.Collision): void {
		((<BlinnPhongMaterial>this.meshSprite.getComponent(Laya.MeshRenderer).sharedMaterial)).albedoColor = this._albedoColor;
		// box.removeSelf();
	}
}
