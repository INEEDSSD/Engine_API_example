...
export class CustomMesh extends BaseScript {
	...
	onAwake(): void {
		...
		//正方体
		var box: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createBox(0.5, 0.5, 0.5), null);
		...
		//球体
		var sphere: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createSphere(0.25, 20, 20), null);
		...
		//圆柱体
		var cylinder: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createCylinder(0.25, 1, 20), null);
		...
		//胶囊体
		var capsule: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createCapsule(0.25, 1, 10, 20), null);
		...
		//圆锥体
		var cone: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createCone(0.25, 0.75), null);
		...
		//平面
		var plane: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createPlane(6, 6, 10, 10), null);
...
export class Tool {
	...
	static linearModel(sprite3D: Sprite3D, lineSprite3D: PixelLineSprite3D, color: Color): void {
		if (sprite3D.getComponent(Laya.MeshFilter)) {
			var meshSprite3D: Laya.Sprite3D = <Laya.Sprite3D>sprite3D;
			var mesh: Mesh = meshSprite3D.getComponent(Laya.MeshFilter).sharedMesh;
	...
	private static DrawTwelveLines(start: Vector3, end: Vector3, rotate: Vector3, sprite3D: Sprite3D): void {
		...
		var cylinder: Laya.Sprite3D = new Laya.Sprite3D();
		let mesh = PrimitiveMesh.createCylinder(0.004, length, 3);
		let meshfilter: Laya.MeshFilter = cylinder.addComponent(Laya.MeshFilter);
		let meshrender: Laya.MeshRenderer = cylinder.addComponent(Laya.MeshRenderer);
