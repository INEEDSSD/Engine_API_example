...
export class MeshLoad extends BaseScript {
	...
	onAwake(): void {
		...
		Laya.loader.load("resources/res/threeDimen/skinModel/LayaMonkey/Assets/LayaMonkey/LayaMonkey-LayaMonkey.lm").then((mesh: Mesh) => {
			...
			var plane: Laya.Sprite3D = new Laya.Sprite3D();
			let planeMesh: Laya.Mesh = PrimitiveMesh.createPlane(6, 6, 10, 10);
			let planeMeshfilter: Laya.MeshFilter = plane.addComponent(Laya.MeshFilter);
			let planeMeshrender: Laya.MeshRenderer = plane.addComponent(Laya.MeshRenderer);
...
export class Tool {
	...
	static linearModel(sprite3D: Sprite3D, lineSprite3D: PixelLineSprite3D, color: Color): void {
		if (sprite3D instanceof Laya.Sprite3D) {
			var meshSprite3D: Laya.Sprite3D = sprite3D;
	...
	static DrawBoundingBox(sprite3D: Sprite3D, sprite: Sprite3D, color: Color): void {
		(<Laya.Sprite3D>sprite3D).getComponent(Laya.MeshRenderer).bounds.getCorners(Tool.corners);
