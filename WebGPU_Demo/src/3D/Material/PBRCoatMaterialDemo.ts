...
export class PBRCoatMaterialDemo extends BaseScript {
	...
	onAwake(): void {
		...
		const row: number = 6;
		this.addSpheresSmoothnessMetallic(sphereMesh, new Vector3(0, 1.5, 0), this.scene, 2, 6, new Color(186 / 255, 110 / 255, 64 / 255, 1.0));
	...
	addPBRSphere(sphereMesh: Mesh, position: Vector3, scene: Scene3D, color: Color, smoothness: number, metallic: number, coat: number, coatR: number, state: boolean): PBRStandardMaterial {
		...
		let meshfilter: Laya.MeshFilter = meshSprite.addComponent(Laya.MeshFilter);
		let meshrender: Laya.MeshRenderer = meshSprite.addComponent(Laya.MeshRenderer);
