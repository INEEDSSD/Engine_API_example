...
export class RealTimeShadow extends BaseScript {
	...
	private onComplete(): void {
		...
		(<SkinnedMeshSprite3D>layaMonkey.getChildAt(0).getChildAt(1)).getComponent(Laya.SkinnedMeshRenderer).castShadow = true;

		// A sphere cast/receive shadow.
		var sphereSprite: Laya.Sprite3D = this.addPBRSphere(PrimitiveMesh.createSphere(0.1), new Vector3(0, 0.2, 0.5), this.scene);
		sphereSprite.getComponent(Laya.MeshRenderer).castShadow = true;
		// sphereSprite.getComponent(Laya.MeshRenderer).receiveShadow = true;
	 ...
	 */
	addPBRSphere(sphereMesh: Mesh, position: Vector3, scene: Scene3D): Laya.Sprite3D {
		var mat: PBRStandardMaterial = new PBRStandardMaterial();
		mat.smoothness = 0.2;

		var meshSprite: Laya.Sprite3D = new Laya.Sprite3D();
...
class RotationScript extends Laya.Script3D {
	...
	onUpdate(): void {
		if (this.rotation)
			(this.owner as Laya.Sprite3D).transform.rotate(this.autoRotateSpeed, false);
	}
}