...
export class PostProcess_Edge extends BaseScript {
    ...
    onAwake(): void {
        ...
        Laya.loader.load("resources/res/threeDimen/skinModel/dude/dude.lh").then(res => {
            let sphere: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createSphere(1), null);
            sphere.name = "Sphere";
            this.scene.addChild(sphere);
            sphere.transform.position = new Vector3(0, 1, 0);

            let plane: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createPlane(), null);
            plane.name = "Plane";
            this.scene.addChild(plane);
            plane.transform.position = new Vector3(0, -0.5, 0);

            let cube: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createBox(1, 1, 1), null);
            cube.name = "Cube";
    ...
    addLight(): void {
        ...
        for (let index = 0; index < dirLightDirections.length; index++) {
            ...
            let dirLight: Laya.Sprite3D = new Laya.Sprite3D();
            let dirLightCom: Laya.DirectionLightCom = dirLight.addComponent(Laya.DirectionLightCom);
