...
export class PhysicsWorld_ConstraintFixedJoint extends BaseScript {
	...
	addbox() {
		//创建盒型MeshSprite3D
		var box: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createBox(1, 1, 1), null);
		...
		//创建盒型MeshSprite3D
		var box2: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createBox(1, 1, 1), null);
		...
		box2.getComponent(Laya.MeshRenderer).sharedMaterial = new BlinnPhongMaterial();
		//创建刚体碰撞器
...
export class FixedEventTest extends Laya.Script {
	...
	private _addForce: Laya.Vector3 = new Laya.Vector3();
	private _addForceCenter: Laya.Vector3 = new Laya.Vector3(0, 0, 0);
	onStart() {
	...
	onUpdate() {
		if (this.fixedConstraint) {
			if ((window as any).PHYSX) {
				this._addForce.setValue(this._addForce.x, this._addForce.y -= 10, this._addForce.z);
				// physx addForce to break the joint
				(this.fixedConstraint.connectedBody as Laya.Rigidbody3D).applyForce(this._addForce, this._addForceCenter);
			} else {
