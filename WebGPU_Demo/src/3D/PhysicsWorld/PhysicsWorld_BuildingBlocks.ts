...
export class PhysicsWorld_BuildingBlocks extends BaseScript {
	...
	onComplete(res: any[]): void {
		...
		var plane: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createPlane(13, 13, 10, 10), null);
	...
	addHorizontalBox(x: number, y: number, z: number): void {
		var box: Laya.Sprite3D = this.createMeshSprite3D(this.mesh1, null);
	...
	addVerticalBox(x: number, y: number, z: number): void {
		var box: Laya.Sprite3D = this.createMeshSprite3D(this.mesh2, null);
		this.scene.addChild(box);
		box.getComponent(Laya.MeshRenderer).sharedMaterial = this.mat;
		box.getComponent(Laya.MeshRenderer).castShadow = true;
		box.getComponent(Laya.MeshRenderer).receiveShadow = true;
