...
export class PhysicsWorld_CollisionFiflter extends BaseScript {
	...
	onAwake(): void {
		...
		//创建平面
		this.plane = this.createMeshSprite3D(PrimitiveMesh.createPlane(20, 20, 10, 10), null);
	...
	addKinematicSphere(): void {
		...
		var sphere: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createSphere(radius), null);
	...
	addBox(): void {
		...
		//设置材质
		box.getComponent(Laya.MeshRenderer).sharedMaterial = new BlinnPhongMaterial();
		Laya.loader.load("resources/res/threeDimen/Physics/rocks.jpg").then((res) => {
			(box.getComponent(Laya.MeshRenderer).sharedMaterial as BlinnPhongMaterial).albedoTexture = res as Texture2D;
		});
	...
	addCapsule(): void {
		...
		var capsule: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createCapsule(raidius, height), null);
		this.scene.addChild(capsule);
		capsule.getComponent(Laya.MeshRenderer).sharedMaterial = new BlinnPhongMaterial();
		Laya.loader.load("resources/res/threeDimen/Physics/wood.jpg").then((res) => {
			(capsule.getComponent(Laya.MeshRenderer).sharedMaterial as BlinnPhongMaterial).albedoTexture = res as Texture2D;
		});
	...
	addCone(): void {
		if ((window as any).PHYSX) return;
		var raidius: number = Math.random() * 0.2 + 0.2;
		var height: number = Math.random() * 0.5 + 0.8;
		//创建圆锥MeshSprite3D
		var cone: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createCone(raidius, height), null);
		this.scene.addChild(cone);
		//设置材质
		cone.getComponent(Laya.MeshRenderer).sharedMaterial = new BlinnPhongMaterial();
		Laya.loader.load("resources/res/threeDimen/Physics/steel2.jpg").then((res) => {
			(cone.getComponent(Laya.MeshRenderer).sharedMaterial as BlinnPhongMaterial).albedoTexture = res as Texture2D;
		});
		//设置位置
	...
	addCylinder(): void {
		...
		var height: number = Math.random() * 0.5 + 0.8;
		//创建圆锥MeshSprite3D
		var cylinder: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createCylinder(raidius, height), null);
		this.scene.addChild(cylinder);
		//设置材质
		cylinder.getComponent(Laya.MeshRenderer).sharedMaterial = new BlinnPhongMaterial();
		Laya.loader.load("resources/res/threeDimen/Physics/steel.jpg").then((res) => {
			(cylinder.getComponent(Laya.MeshRenderer).sharedMaterial as BlinnPhongMaterial).albedoTexture = res as Texture2D;
		});
	...
	addSphere(): void {
		...
		this.scene.addChild(sphere);
		//设置材质
		sphere.getComponent(Laya.MeshRenderer).sharedMaterial = new BlinnPhongMaterial();
		Laya.loader.load("resources/res/threeDimen/Physics/plywood.jpg").then((res) => {
			(sphere.getComponent(Laya.MeshRenderer).sharedMaterial as BlinnPhongMaterial).albedoTexture = res as Texture2D;
		});
		var pos: Vector3 = sphere.transform.position;
		pos.setValue(Math.random() * 4 - 2, 10, Math.random() * 4 - 2);
		sphere.transform.position = pos;
