...
export class PhysicsWorld_CompoundCollider extends BaseScript {
	...
	onAwake(): void {
		...
		var plane: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createPlane(20, 20, 20, 10), null);
	...
	addTable(): void {
		...
		Laya.loader.load("resources/res/threeDimen/Physics/table.lm").then((res) => {
			var table: Laya.Sprite3D = this.createMeshSprite3D(res, null);
	...
	addObject(): void {
		...
		Laya.loader.load("resources/res/threeDimen/Physics/object.lm").then((res) => {
			...
			var object: Laya.Sprite3D = this.createMeshSprite3D(res, null);
