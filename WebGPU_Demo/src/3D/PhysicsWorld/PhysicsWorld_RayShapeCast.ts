...
export class PhysicsWorld_RayShapeCast extends BaseScript {
	...
	private rayDir: Vector3 = new Vector3(0, 0, -1);
	...
	onAwake(): void {
		...
		//添加地面
		var plane: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createPlane(20, 20, 10, 10), null);
	...
	stypeFun2(castType: number = 0) {
		if (this.hitResult.succeeded)
			((<BlinnPhongMaterial>((<Laya.Sprite3D>this.hitResult.collider.owner)).getComponent(Laya.MeshRenderer).sharedMaterial)).albedoColor = new Color(1.0, 1.0, 1.0, 1.0);

		if (this.hitResults.length > 0) {
			for (var i: number = 0, n: number = this.hitResults.length; i < n; i++)
				((<BlinnPhongMaterial>((<Laya.Sprite3D>this.hitResults[i].collider.owner)).getComponent(Laya.MeshRenderer).sharedMaterial)).albedoColor = new Color(1.0, 1.0, 1.0, 1.0);
		...
		switch (this.castType) {
			case 0:
				...
				if (this.castAll) {
					...
					for (i = 0, n = this.hitResults.length; i < n; i++)
						//将射线碰撞到的物体设置为红色
						((<BlinnPhongMaterial>((<Laya.Sprite3D>this.hitResults[i].collider.owner)).getComponent(Laya.MeshRenderer).sharedMaterial)).albedoColor = new Color(1.0, 0.0, 0.0, 1.0);
				} else {
					//进行射线检测,检测第一个碰撞物体
					this.scene.physicsSimulation.rayCast(this.ray, this.hitResult);
					//将检测到的物体设置为红色
					((<BlinnPhongMaterial>((<Laya.Sprite3D>this.hitResult.collider.owner)).getComponent(Laya.MeshRenderer).sharedMaterial)).albedoColor = new Color(1.0, 0.0, 0.0, 1.0);
				}
				break;
			case 1:
				//创建盒型碰撞器
				...
				for (i = 0; i < 21; i++) {
					//创建进行射线检测的盒子精灵
					var boxSprite: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createBox(1.0, 1.0, 1.0), null);
			...
			case 2:
				...
				for (i = 0; i < 41; i++) {
					var sphereSprite: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createSphere(0.5), null);
			...
			case 3:
				...
				for (i = 0; i < 41; i++) {
					var capsuleSprite: Laya.Sprite3D = this.createMeshSprite3D(PrimitiveMesh.createCapsule(0.25, 1.0), null);
