...
class TriggerCollisionScript extends Laya.Script {
	...
	//开始触发时执行
	onTriggerEnter(other: Laya.PhysicsColliderComponent): void {
		((<BlinnPhongMaterial>((<MeshRenderer>((<Laya.Sprite3D>this.owner)).getComponent(Laya.MeshRenderer))).sharedMaterial)).albedoColor = new Color(0.0, 1.0, 0.0, 1.0);
	}

	//持续触发时执行
	onTriggerStay(other: Laya.PhysicsColliderComponent): void {
	}

	//结束触发时执行
	onTriggerExit(other: Laya.PhysicsColliderComponent): void {
		((<BlinnPhongMaterial>((<MeshRenderer>((<Laya.Sprite3D>this.owner)).getComponent(Laya.MeshRenderer))).sharedMaterial)).albedoColor = new Color(1.0, 1.0, 1.0, 1.0);
	}

	//开始碰撞时执行
	onCollisionEnter(collision: Collision): void {
		if (collision.other.owner === this.kinematicSprite)
			((<BlinnPhongMaterial>((<MeshRenderer>((<Laya.Sprite3D>this.owner)).getComponent(Laya.MeshRenderer))).sharedMaterial)).albedoColor = new Color(0.0, 0.0, 0.0, 1.0);
	}
